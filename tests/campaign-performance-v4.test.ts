...
describe("V4 marketing attribution and campaign performance", () => {
  ...
  it("counts one Lead with two Opportunities once in the funnel", () => { const duplicate={...opportunities[0],id:"o4",stage:"Proposal" as const}; const funnel=calculateMarketingFunnel(leads,[...opportunities,duplicate]); assert.equal(funnel[2].count,3); assert.equal(calculatePerformance(leads,[...opportunities,duplicate]).opportunities,4); });
  it("counts one Lead with multiple Won Opportunities as one Won Lead", () => { const secondWon={...opportunities[1],id:"o4",value:250}; const result=calculatePerformance(leads,[...opportunities,secondWon]); const funnel=calculateMarketingFunnel(leads,[...opportunities,secondWon]); assert.equal(result.wonOpportunities,2); assert.equal(result.wonLeads,1); assert.equal(funnel[3].count,1); });
  it("keeps Lead to Opportunity conversion at or below 100 percent with duplicate Opportunities", () => { const repeated=[...opportunities,...Array.from({length:5},(_,index)=>({...opportunities[0],id:`duplicate-${index}`}))]; const result=calculatePerformance(leads,repeated); assert.equal(result.leadsWithOpportunity,3); assert.equal(result.leadToOpportunityRate,75); assert.ok((result.leadToOpportunityRate ?? 0)<=100); });
});
