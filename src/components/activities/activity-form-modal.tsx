    ...
    <div className="my-6 w-full max-w-3xl rounded-2xl bg-white shadow-2xl">
      ...
      <form onSubmit={handleSubmit} noValidate>
        <div className="grid gap-5 px-6 py-5 sm:grid-cols-2">
          ...
          <label className={labelClass}>预算（CNY）<input name="budget" type="number" min="0" value={draft.budget} onChange={(e) => update("budget", Number(e.target.value))} className={inputClass} />{errors.budget && <span className="mt-1 block text-xs text-rose-600">{errors.budget}</span>}</label>
